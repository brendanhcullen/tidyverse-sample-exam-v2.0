...
You could also use `dplyr::across()` and a named list of functions!
...
The file `home-range-database.csv` is currently saved in the root directory of the project, not in a subdirectory called `data`, as the code above would suggest. You could either create the `data` folder and move the data file there, or you could update the code that imports the data (as I demonstrate below):
...
Note that you need to add an indentation after `html_document:` and add `toc: TRUE`