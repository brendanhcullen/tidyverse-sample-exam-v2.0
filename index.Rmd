...
library(metathis)
meta() %>% 
  meta_description("A solution guide to a sample tidyverse exam for the RStudio Instructor Training and Certification Program") %>% 
  ...
  meta_social(
    ...
    image = "https://raw.githubusercontent.com/brendanhcullen/tidyverse-sample-exam-v2.0/master/img/tidyverse_key.png",
...
:::toggle
By default, the code for each solution is hidden. You can toggle the code on and off by clicking `Code`/`Hide` at the top right of each chunk or `Code > Show All Code` at the top right of this document.
:::
